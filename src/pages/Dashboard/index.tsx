...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    async function loadFoods(): Promise<void> {
      let endpointUrl = '/foods';

      if (selectedCategory) {
        endpointUrl += `?category_like=${selectedCategory}`;
      } else {
        endpointUrl += `?name_like=${searchValue}`;
      }

      try {
        const response = await api.get<Food[]>(endpointUrl);

        const fetchedFoods = response.data;

        const fetchedFoodsWithPrice = fetchedFoods.map(food => ({
          ...food,
          formattedPrice: formatValue(food.price),
        }));

        setFoods(fetchedFoodsWithPrice);
      } catch (error) {
        console.log(error);
      }
    }
